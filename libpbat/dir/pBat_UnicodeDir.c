...
wchar_t* pBat_GetNextLevel(wchar_t* up, wchar_t* base, int* jok)
{
    int joker = 0;
    wchar_t* prev = NULL;
...
static int __inline__ pBat_FileExists_W(const wchar_t* ptrName)
{
    int iAttrib;
...
static int __inline__ pBat_DirExists_W(const wchar_t *ptrName)
{
    int iAttrib;
...
static int __inline__ pBat_IsRegExpTrivial(wchar_t* exp)
{
    if (wcspbrk(exp, L"*?")) {
        return 0;
    } else
...
static time_t __inline__  pBat_ToTime_t(FILETIME* ft)
{
    ULARGE_INTEGER ull;
...
static void __inline__ pBat_FillStat(struct stat* st, WIN32_FIND_DATAW* data)
{
    st->st_ctime = pBat_ToTime_t(&(data->ftCreationTime));
    st->st_atime = pBat_ToTime_t(&(data->ftLastAccessTime));
    st->st_mtime = pBat_ToTime_t(&(data->ftLastWriteTime));

    st->st_mode = data->dwFileAttributes;
    st->st_size = ((size_t)data->nFileSizeLow) | (((size_t)data->nFileSizeHigh) << sizeof(DWORD));
}

static void __inline__ pBat_GetStat(struct stat* st, wchar_t* wname)
{
    ...
    st->st_ctime = pBat_ToTime_t(&(attr.ftCreationTime));
    st->st_atime = pBat_ToTime_t(&(attr.ftLastAccessTime));
    st->st_mtime = pBat_ToTime_t(&(attr.ftLastWriteTime));

    st->st_mode = attr.dwFileAttributes;
    st->st_size = ((size_t)attr.nFileSizeLow) | (((size_t)attr.nFileSizeHigh) << sizeof(DWORD));
}

static FILELIST* pBat_AddMatch(wchar_t* wname, FILELIST* files, struct match_args_t* arg, WIN32_FIND_DATAW* data)
{
    FILELIST *file,
             block;
    char *name;
    ...
    if (arg->callback) {
        ...
        if (!(arg->flags & PBAT_SEARCH_NO_STAT)) {
            ...
            if (data == NULL) {
                ...
                pBat_GetStat(&(block.stFileStats), wname);

            } else {

                pBat_FillStat(&(block.stFileStats), data);
    ...
    if (!(arg->flags & PBAT_SEARCH_NO_STAT)) {
        if (data == NULL) {
            ...
            pBat_GetStat(&(file->stFileStats), wname);

        } else {

            pBat_FillStat(&(file->stFileStats), data);
...
static FILELIST* pBat_GetMatch(wchar_t* restrict base, wchar_t* restrict up, struct match_args_t* arg)
{
    ...
    if (base == NULL) {
        ...
        if ((arg->flags &
            ...
            if (!(arg->flags & PBAT_SEARCH_DIR_MODE)
                ...
                if ((tmp = pBat_AddMatch(up, ret, arg, NULL)) == NULL)
                    goto err;

                arg->files = tmp;
            }

            ret = pBat_GetMatch(up, L"*", arg);
        ...
        if (pBat_DirExists_W(up) || pBat_FileExists_W(up)) {
    ...
    if (!joker) {
        ...
        if (pBat_FileExists_W(path)) {
            ...
            if (up == NULL) {
                ...
                if ((tmp = pBat_AddMatch(path, ret, arg, NULL)) == NULL)
                    goto err;
    ...
    while (loop) {
        ...
        if (!(ent.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)
            ...
            if (up == NULL) {
                ...
                if ((tmp = pBat_AddMatch(path, ret, arg, &ent)) == NULL)
                    goto err;
            ...
            if (up == NULL) {
                ...
                if ((arg->flags & PBAT_SEARCH_RECURSIVE)
                    ...
                    arg->files = ret;
                    ret = pBat_GetMatch(path, item, arg);
...
LIBPBAT LPFILELIST  pBat_GetMatchFileList(char* lpPathMatch, int iFlag)
{
    ...
    file = pBat_GetMatch(NULL, wpath, &args);
