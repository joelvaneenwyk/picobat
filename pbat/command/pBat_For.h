...
int pBat_CmdFor(char* lpCommand, PARSED_LINE** lpplLine);
...
#define PBAT_FORL_BEGIN 0
#define PBAT_FORL_INC   1
#define PBAT_FORL_END   2

int pBat_CmdForL(ESTR* lpInput, PARSED_LINE* lpplLine, char cVarName);
/* this handle the ``for /L'' loop */


int pBat_ForMakeInfo(char* lpOptions, FORINFO* lpfrInfo);
/* this makes token from a parameter command
   i.e. from a string like "tokens=1,3,4 delims=, "
*/

#define PBAT_FOR_SPEC_INVALID   0
#define PBAT_FOR_SPEC_USEBACKQ  1
#define PBAT_FOR_SPEC_TOKENS    2
#define PBAT_FOR_SPEC_DELIMS    3
#define PBAT_FOR_SPEC_SKIP      4
#define PBAT_FOR_SPEC_EOL       5
...
#define PBAT_FOR_TOKEN_LIST 0 /* a value for the ',' token operator */
#define PBAT_FOR_TOKEN_CAT  1 /* a value for the '.' token operator */

int  pBat_ForMakeTokens(char* lpToken, FORINFO* lpfrInfo);
...
int  pBat_CmdForDeprecatedWrapper(ESTR* lpMask, ESTR* lpDir, char* lpAttribute,
